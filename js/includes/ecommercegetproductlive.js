...
var myfunc = async function(obj) {
  ...
  try {
    ...
    if(decrypted.data) {
      ...
      const sql = "SELECT * FROM tbl_product WHERE product_status=10 ORDER BY product_id ASC";
      ...
      if(res&&res.rows&&res.rows.length) {
        ...
        for(var x in live) {
          try {
            if(live[x].product_photos) {
              live[x].product_photos = JSON.parse(live[x].product_photos);
              live[x].product_photos1 = live[x].product_photos[0];
            }
          } catch(e) {
            console.log(e);
          }
      ...
      var retdata = {success:1, live:live}
