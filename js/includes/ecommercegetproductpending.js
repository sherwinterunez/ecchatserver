...
var myfunc = async function(obj) {
  ...
  try {
    ...
    if(decrypted.data) {
      ...
      const sql = "SELECT * FROM tbl_product WHERE product_status=1 ORDER BY product_id ASC";
      ...
      if(res&&res.rows&&res.rows.length) {
        ...
        for(var x in pending) {
          try {
            if(pending[x].product_photos) {
              pending[x].product_photos = JSON.parse(pending[x].product_photos);
              pending[x].product_photos1 = pending[x].product_photos[0];
            }
          } catch(e) {
            console.log(e);
          }
      ...
      var retdata = {success:1, pending:pending}
